 ...
 * @ignore
 */
const RtcEngineEvent = new _reactNative.NativeEventEmitter(AgoraRtcEngineModule);
...
class RtcEngine {
  ...
  static async createWithContext(context) {
    ...
    let deepARKey;
    if (context.deepARKey) {
      deepARKey = context.deepARKey;
    }
    let deepARWidth;
    if (context.deepARWidth) {
      deepARWidth = context.deepARWidth;
    }
    let deepARHeight;
    if (context.deepARHeight) {
      deepARHeight = context.deepARHeight;
    }
    await RtcEngine._callMethod('create', {
      config: {
        ...
        areaCode: areaCode,
        deepARKey: deepARKey,
        deepARWidth: deepARWidth,
        deepARHeight: deepARHeight
      },
      appType: 8
    });
   ...
   * @param listener The [`RtcEngineEvents`]{@link RtcEngineEvents} handler.
   */
  removeListener(event, listener, subscription) {
   ...
   */
  enableRemoteSuperResolution(uid, enabled) {
   ...
   */
  startEchoTest(intervalInSeconds, config) {
    console.assert(intervalInSeconds === undefined || config === undefined, 'Only need one of the params');
  ...
  setLocalAccessPoint(ips, domain) {
    return RtcEngine._callMethod('setLocalAccessPoint', {
      ips,
      domain
    });
  }
   ...
   *
   */
  enableVirtualBackground(enabled, backgroundSource) {
   ...
   *
   */
  takeSnapshot(channel, uid, filePath) {
  ...
  enableContentInspect(enabled, config) {
    return RtcEngine._callMethod('enableContentInspect', {
      enabled,
      config
    });
  }
  ...
  enableWirelessAccelerate(enabled) {
    return RtcEngine._callMethod('enableWirelessAccelerate', {
      enabled
    });
  }
   ...
   *
   */
  setAgoraLibPath(path) {
    return RtcEngine._callMethod('setAgoraLibPath', {
      path
    });
  }
  ...
  startRecording(config) {
    return RtcEngine._callMethod('startRecording', {
      config
    });
  }
  ...
  startRtmpStreamWithoutTranscoding(url) {
    return RtcEngine._callMethod('startRtmpStreamWithoutTranscoding', {
      url
    });
  }
   ...
   */
  stopRtmpStream(url) {
    return RtcEngine._callMethod('stopRtmpStream', {
      url
    });
  }
  ...
  updateRtmpTranscoding(transcoding) {
    return RtcEngine._callMethod('updateRtmpTranscoding', {
      transcoding
    });
  }
  ...
  setAVSyncSource(channelId, uid) {
    return RtcEngine._callMethod('setAVSyncSource', {
      channelId,
      uid
    });
  }
   ...
   * - Void if the method call succeeds.
   */
  switchDeepArEffect(name, slot) {
    return RtcEngine._callMethod('switchDeepArEffect', {
      name,
      slot
    });
  }
